...
// ---------------- Chat attachment storage (offer negotiation messages) ----------------
const chatAttachmentDir = path.join(__dirname, "../../public/chat_attachments");
...
const chatAttachmentStorage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, chatAttachmentDir),
  filename: (req, file, cb) => {
    const unique = uuidv4();
    const ext = path.extname(file.originalname);
    cb(null, `${unique}${ext}`);
...
// Wider filter than images-only: resumes are commonly PDF or Word docs.
const chatAttachmentFileFilter = (
  ...
  if (allowedMimeTypes.includes(file.mimetype)) {
    ...
    cb(new HttpError(400, "Invalid file type. Only images, PDF, or Word documents are allowed."));
  }
};
...
// Combined uploader for sending a chat message: optional single "attachment".
export const offerMessageUpload = chatAttachmentUpload.fields([
